...
This document describes the technical architecture of the Daichi Cloud Climate custom component for Home Assistant.

## Directory Structure

```text
custom_components/daichi_cloud_integration/
...
│   ├── __init__.py          # Exports DaichiCloudClimateDataUpdateCoordinator
...
│   ├── __init__.py          # Exports DaichiCloudClimateEntity
...
- `base.py` - Main coordinator class (`DaichiCloudClimateDataUpdateCoordinator`)
...
**Key class:** `DaichiCloudClimateDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `DaichiCloudClimateApiClient`
...
- `DaichiCloudClimateConfigFlowHandler` (main flow)
- `DaichiCloudClimateOptionsFlow` (options)
...
**Key class:** `DaichiCloudClimateEntity` (in `entity/base.py`)
...
2. `DaichiCloudClimateEntity` for common functionality
...
1. Create directory: `custom_components/daichi_cloud_integration/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `DaichiCloudClimateEntity`
