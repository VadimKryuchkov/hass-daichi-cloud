...
This document describes all configuration options and settings available in the Daichi Cloud Climate custom integration.
...
### `daichi_cloud_integration.example_service`
...
```yaml
service: daichi_cloud_integration.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: daichi_cloud_integration.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Daichi Cloud Climate Alert
  ...
  input:
    sensor_entity:
      ...
      selector:
        entity:
          domain: sensor
          integration: daichi_cloud_integration
    threshold:
      name: Threshold
      selector:
        number:
...
- [GitHub Issues](https://github.com/VadimKryuchkov/hass-daichi-cloud/issues) - Report problems
